...
def build_oak_pipeline():
    ...
    rgbQ = rgbOutput.createOutputQueue(maxSize=1, blocking=False)
    depthQ = stereo.depth.createOutputQueue(maxSize=1, blocking=False)

    return pipeline, rgbQ, depthQ
...
def main():
    ...
    if not args.no_camera:
        try:
            pipeline, rgbQ, depthQ = build_oak_pipeline()
            pipeline.start()
            print("OAK camera connected (depth in mm)")
        except Exception as e:
    ...
    try:
        while True:
            ...
            # --- Depth side panel (mm values, clipped to 15m) ---
            if depth_frame is not None:
                ...
                depth_color[depth_frame == 0] = 0  # black out invalid pixels
